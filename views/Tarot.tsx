...
const Tarot: React.FC<TarotProps> = ({ onBack }) => {
  ...
  const pullCard = async () => {
    ...
    try {
      ...
      console.error(e);
      setLoading(false);
      setError("The cards are clouded. The connection to the beyond has faded.");
    }
  };
        ...
        <div className="flex-1 flex flex-col items-center">
          {!card && !loading && (
            <div className="flex-1 flex flex-col items-center justify-center text-center animate-in fade-in duration-1000">
              ...
              <p className="text-sm text-gray-400 mb-6 max-w-[300px] leading-relaxed font-light">Clear your mind. Silently ask your question to the universe before you pull.</p>

              {error && (
                <div className="mb-8 p-4 border border-purple-900/30 bg-purple-950/10 rounded-2xl max-w-xs">
                  <p className="text-xs text-purple-300 italic">{error}</p>
                </div>
              )}

              <button onClick={pullCard} className="group relative w-56 h-80 card-flip cursor-pointer focus:outline-none">
                <div className="w-full h-full bg-black rounded-2xl glow-mystic transition-all group-hover:scale-105 group-hover:shadow-[0_0_50px_rgba(147,51,234,0.4)] relative overflow-hidden">
                  <img src={CARD_BACK_URL} alt="Tarot Back" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
          ...
          {(loading || card) && (
            <div className="flex-1 flex flex-col items-center w-full max-w-4xl mx-auto py-8">
              <div id="tarot-card-area" className={`w-64 h-96 card-flip mb-12 transition-transform duration-1000 ${loading ? 'animate-vertical-float scale-110' : ''}`}>
                <div className={`card-inner glow-mystic ${isFlipped ? 'is-flipped' : ''} ${card?.isReversed ? 'rotate-180' : ''}`}>
                  <div className="card-back bg-black border border-purple-900/40">
                      <img src={CARD_BACK_URL} alt="Tarot Back" className="w-full h-full object-cover opacity-90" />
                      {loading && (
              ...
              {loading && (
                <p className="text-xs text-purple-400 animate-pulse tracking-[0.4em] uppercase font-medium">Whispering to the ether...</p>
              )}

              {card && !loading && isFlipped && (
                <div id="tarot-result" className="w-full space-y-12 animate-in fade-in slide-in-from-bottom duration-1000 pb-20">
                  <div className="text-center">
                    <span className="text-[10px] uppercase tracking-[0.4em] text-purple-500 block mb-2 font-bold">Your Orientation</span>
                    <h3 className="text-4xl font-mystical text-white">{card.name} <span className="text-gray-500 opacity-50 font-sans text-xl ml-2">{card.isReversed ? 'Reversed' : 'Upright'}</span></h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 px-4">
                    ...
                    <div className="p-8 bg-purple-900/10 rounded-[2rem] border border-purple-900/30 shadow-xl shadow-purple-950/10 flex flex-col justify-center">
                      ...
                      <p className="text-lg text-gray-100 font-light italic leading-relaxed">{card.interpretation}</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-center gap-6 pt-8">
                    <ShareButton 
                      elementId="tarot-result" 
                      title="My Tarot Reading" 
                      text={`I pulled the ${card.name}. The cards have shared their wisdom.`} 
                    />
                    <button onClick={handleReset} className="text-[10px] uppercase tracking-[0.4em] text-gray-600 hover:text-purple-400 transition-colors py-4 px-10 border border-purple-900/20 hover:border-purple-500 rounded-full">Reshuffle and Ask Again</button>
                  </div>
                </div>
              )}
            </div>
